...
if __name__ == "__main__":
    ...
    num_samples = 2# 120  # Increased sample count to handle higher dimensions smoothly
    dimensions = 6   # [kp_vel, ki_vel, kp_att, kd_att, kp_alt, kd_alt]

    # Establish the bounding envelope array arrays matching the table above
    gains_lower_bounds = np.array([0.02,  0.005, 1.0, 0.1,  5.0,  2.0])
    gains_upper_bounds = np.array([0.40,  0.150, 5.0, 1.0, 25.0, 15.0])

    sampler = qmc.LatinHypercube(d=dimensions, seed=42)
    sample = sampler.random(n=num_samples)
    # Scale the raw LHS matrix across our specific gain bounds
    scaled_gains_matrix = qmc.scale(sample, gains_lower_bounds, gains_upper_bounds)

    # 2. Execute the LHS Experiment Loop
    compiled_results = []

    print(f"Executing 6D LHS batch profile across {num_samples} design points...")
    for i, full_gains in enumerate(scaled_gains_matrix):
        try:
        # 1. Run the simulation
            outputs = run_drone_acoustic_simulation(
                full_gains, dryden_ts, velocity_schedule, time_steps, dt
            )
            spl_fine, pa, rpm_result, hist_state, hist_controls, hist_accel, hist_gusts = outputs
            
            # 2. Check if the output state is corrupted with NaNs or Infinities
            if np.any(np.isnan(hist_state)) or np.any(np.isinf(hist_state)):
                raise ValueError("Simulation diverged internally (NaN/Inf detected).")
                
            annoyance_time_signal = np.asarray(pa["annoyance_PA"])
            max_annoyance = float(np.nanmax(annoyance_time_signal))
            mean_annoyance = float(np.nanmean(annoyance_time_signal))
            full_signal = annoyance_time_signal

        except Exception as e:
            # 3. Catch crashes (like the CubicSpline one) or our custom divergence check
            print(f"  -> Run LHS_{i+1:02d} went unstable! Skipping acoustics pipeline.")
            
            # Assign a severe penalty score so it stands out as a failure on your charts
            max_annoyance = 99.0  # Or slightly above your worst successful run
            mean_annoyance = 99.0
            full_signal = np.full_like(time_steps, 99.0)

    # Compile everything safely into the database rows

        # # Run the simulation directly using the 6 elements of full_gains
        # outputs = run_drone_acoustic_simulation(
        #     full_gains, dryden_ts, velocity_schedule, time_steps, dt
        # )
        # spl_fine, pa, rpm_result, hist_state, hist_controls, hist_accel, hist_gusts = outputs
        ...
        # Compile everything into the database rows
        compiled_results.append({
            'Run_ID': f"LHS_{i+1:02d}",
            ...
            # Summary scalar features for plots
            'Max_Annoyance': float(np.nanmax(annoyance_time_signal)),
            'Mean_Annoyance': float(np.nanmean(annoyance_time_signal)),
    ...
    print("Batch complete. High-dimensional dataset generated.")

    # Create a comprehensive High-Dimensional Parallel Coordinates Plot
    fig, ax = plt.subplots(figsize=(15, 7))
    plt.title("6D Parallel Coordinates: Mapping Multi-Loop Drone Gains to Human Annoyance", 
            fontsize=14, fontweight='bold', pad=15)

    # 1. Setup performance color metrics and normalizer
    norm_metric = df['Max_Annoyance']
    # Create a strict normalization mapping from the minimum to maximum annoyance found
    norm = colors.Normalize(vmin=norm_metric.min(), vmax=norm_metric.max())
    cmap = cm.get_cmap('turbo') # 'turbo' or 'coolwarm' work beautifully here

    # Label vectors for mapping columns
    gain_keys = ['Kp_Velocity', 'Ki_Velocity', 'Kp_Attitude', 'Kd_Attitude', 'Kp_Altitude', 'Kd_Altitude', 'Max_Annoyance']
    display_labels = ['$K_{p, v}$', '$K_{i, v}$', '$K_{p, att}$', '$K_{d, att}$', '$K_{p, alt}$', '$K_{d, alt}$', 'Peak Annoyance [PA]']

    # Normalize EACH column to its own local 0-1 scale strictly for visual alignment in the chart
    df_normalized = df[gain_keys].copy()
    for col in gain_keys:
        df_normalized[col] = (df[col] - df[col].min()) / (df[col].max() - df[col].min())

    # 2. Plot the lines color-coded by their raw metric value
    x_indices = np.arange(len(gain_keys))
    for idx, row in df_normalized.iterrows():
        line_y_coords = [row[k] for k in gain_keys]
        
        # Extract the raw annoyance value of this specific run to determine its true color
        raw_annoyance_val = df.loc[idx, 'Max_Annoyance']
        line_color = cmap(norm(raw_annoyance_val))
        
        ax.plot(x_indices, line_y_coords, color=line_color, linewidth=2.0, alpha=0.7)

    # Set visual markers
    ax.set_xticks(x_indices)
    ax.set_xticklabels(display_labels, fontsize=12, fontweight='bold')
    ax.set_ylabel('Normalized Range Context [Min to Max]', fontsize=11)
    ax.grid(True, axis='x', linestyle='-', color='black', alpha=0.4)

    # Overlay local min/max values text at the top and bottom of each axis to make it readable
    for x_idx, key in enumerate(gain_keys):
        ax.text(x_idx, 1.02, f"Max:\n{df[key].max():.2f}", ha='center', va='bottom', fontsize=9, color='darkred')
        ax.text(x_idx, -0.02, f"Min:\n{df[key].min():.2f}", ha='center', va='top', fontsize=9, color='darkblue')

    # --- 3. THE COLORBAR ADDITION ---
    # Create a dummy mappable object that tells matplotlib how the color bar connects to our data range
    sm = cm.ScalarMappable(cmap=cmap, norm=norm)
    sm.set_array([]) # Passing an empty array is a required matplotlib syntax workaround

    # Draw the colorbar onto the right side of the active plotting window
    cbar = fig.colorbar(sm, ax=ax, pad=0.05, shrink=0.8)
    cbar.set_label('Psychoacoustic Annoyance Index [PA]', rotation=270, labelpad=20, fontsize=11, fontweight='bold')
    ...
    # # Setup performance color metrics
    # norm_metric = df['Max_Annoyance']
    # norm_pa = (norm_metric - norm_metric.min()) / (norm_metric.max() - norm_metric.min())
    # cmap = plt.cm.get_cmap('turbo') # 'turbo' gives a highly visible spectrum gradient
    ...
    # for idx, row in df_normalized.iterrows():
    #     line_y_coords = [row[k] for k in gain_keys]
    #     ax.plot(x_indices, line_y_coords, color=cmap(norm_pa[idx]), linewidth=2.0, alpha=0.7)
    ...
    df_scalars_only = df.drop(columns=['Full_Annoyance_Array'])

    # Run the pairplot using your filtered DataFrame
    sns.pairplot(
        df_scalars_only, 
        vars=['Kp_Velocity', 'Kp_Attitude', 'Kd_Attitude'], 
        hue='Max_Annoyance', 
        palette='turbo'
    )
    ...
    # # Create a 2-Panel Diagnostic Dashboard
    # fig = plt.figure(figsize=(14, 6))

    # # Panel A: Parallel Coordinates Plot
    # ax1 = fig.add_subplot(121)
    # plt.title("Parallel Coordinates: Gain Traces to Annoyance Mapping", fontsize=12, fontweight='bold')

    # # Use the scalar 'Max_Annoyance' column for the color mapping normalization
    # norm_metric = df['Max_Annoyance']
    # norm_pa = (norm_metric - norm_metric.min()) / (norm_metric.max() - norm_metric.min())
    # cmap = plt.cm.get_cmap('coolwarm') 

    # for idx, row in df.iterrows():
    #     # Pass the scalar value (row['Max_Annoyance']) to the line coordinates
    #     line_y = [row['Kp_Velocity'], row['Ki_Velocity'], row['Max_Annoyance']]
    #     line_x = [0, 1, 2]
    #     ax1.plot(line_x, line_y, color=cmap(norm_pa[idx]), linewidth=2, alpha=0.8)

    # ax1.set_xticks([0, 1, 2])
    # ax1.set_xticklabels(['$K_p$ Velocity', '$K_i$ Velocity', 'Peak Annoyance (PA)'], fontsize=11, fontweight='bold')
    # ax1.set_ylabel('Parameter Variable Values')
    # ax1.grid(True, axis='x', linestyle='-', color='black', alpha=0.3)

    # # Panel B: 2D Design Space Scatter Map
    # ax2 = fig.add_subplot(122)
    # plt.title("Design Space Topography: Sensitivity Mapping", fontsize=12, fontweight='bold')

    # scatter = ax2.scatter(df['Kp_Velocity'], df['Ki_Velocity'], 
    #                     c=df['Max_Annoyance'], s=df['Max_Annoyance']*10, 
    #                     cmap='coolwarm', edgecolors='black', alpha=0.9, zorder=3)

    # ax2.set_xlabel('$K_p$ Velocity Gain (Proportional)', fontsize=11)
    # ax2.set_ylabel('$K_i$ Velocity Gain (Integral)', fontsize=11)
    # cbar = plt.colorbar(scatter, ax=ax2)
    # cbar.set_label('Peak Psychoacoustic Annoyance [PA]', rotation=270, labelpad=15, fontweight='bold')
    ...
    # radar plots 
    # 1. Identify the best and worst runs from your LHS DataFrame
    best_run = df.loc[df['Max_Annoyance'].idxmin()]
    worst_run = df.loc[df['Max_Annoyance'].idxmax()]

    # Variables to plot (excluding the Run_ID and arrays)
    labels = ['$K_{p, v}$', '$K_{i, v}$', '$K_{p, att}$', '$K_{d, att}$', '$K_{p, alt}$', '$K_{d, alt}$']
    gain_keys = ['Kp_Velocity', 'Ki_Velocity', 'Kp_Attitude', 'Kd_Attitude', 'Kp_Altitude', 'Kd_Altitude']
    num_vars = len(labels)

    # 2. Normalize values between 0 and 1 based on your global LHS bounds
    # This ensures that '1.0' always means the maximum possible gain tested
    best_norm = []
    worst_norm = []
    for i, key in enumerate(gain_keys):
        g_min = df[key].min()
        g_max = df[key].max()
        best_norm.append((best_run[key] - g_min) / (g_max - g_min))
        worst_norm.append((worst_run[key] - g_min) / (g_max - g_min))

    # Close the circular loop for the polar plot by repeating the first element
    best_norm += best_norm[:1]
    worst_norm += worst_norm[:1]
    ...
    # Draw the 'Best' profile (Quiet Setup)
    ax.plot(angles, best_norm, color='#1f77b4', linewidth=2, label=f"Best Setup (PA: {best_run['Max_Annoyance']:.1f})")
    ax.fill(angles, best_norm, color='#1f77b4', alpha=0.25)

    # Draw the 'Worst' profile (Annoying Setup)
    ax.plot(angles, worst_norm, color='#d62728', linewidth=2, label=f"Worst Setup (PA: {worst_run['Max_Annoyance']:.1f})")
    ax.fill(angles, worst_norm, color='#d62728', alpha=0.15)

    # Fix labels to spokes
    ax.set_xticks(angles[:-1])
    ax.set_xticklabels(labels, fontsize=12, fontweight='bold')

    # Configure visual bounds
    ax.set_ylim(0, 1)
    ax.set_yticklabels([]) # Hide 0-1 numbers to keep it clean
    plt.title("Tuning Profile Architecture Comparison\n(Normalized Parameter Envelopes)", fontsize=14, fontweight='bold', pad=20)
