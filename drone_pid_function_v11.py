...
def evaluate_drone_codesign(X, dt, t_end, debug=False):
    ...
    Parameters:
    -----------
    X : array-like of shape (12,)
        [R_rotor, L_arm, v_max, t_climb, vx_max, vy_max, Kp_vel, Ki_vel, Kp_att, Kd_att, Kp_alt, Kd_alt]
    """
    # 1. UNPACK THE DESIGN VECTOR (Indices 0 to 5)
    R_rotor   = float(X[0])
    L_arm     = float(X[1])
    v_max     = float(X[2])   # (This is v_z_max)
    t_climb   = float(X[3])
    vx_max    = float(X[4])   # Added New Design Variable
    vy_max    = float(X[5])   # Added New Design Variable for future expansion
    
    # Extract your 6 control gains (Now shifted from index 6 to 12)
    ...
    dryden_ts = generate_dryden_time_series(
        ...
        params=DrydenParams(
            ...
            arm_length=L_arm,  # Linked dynamically to current design arm length!
    ...
    # Note: If your trajectory generator doesn't accept vy_max yet, it will just ignore it safely
    ...
    # waypoints, time_steps, v_x_ref, v_z_ref = generate_dynamic_trajectory(
    #     vx_max, v_max, t_climb, dt=dt, t_total=t_end
    # )
    ...
    if debug:
        ...
        if check_tracking_performance(hist_state, v_z_ref):
            ...
            return {"objective": 99.0, "feasible": False}, None, None, None, None
        ...
        return {"objective": float(annoyance_dose), "feasible": True}, hist_state, time_steps, v_x_ref, v_z_ref, pa
        ...
        try:
            ...
            print(f"Current Evaluation Annoyance: {annoyance_dose:.4f}")
...
def plot_comprehensive_diagnostics(hist_state, time_steps, v_x_ref, v_z_ref, pa):
    """
...
if __name__ == "__main__":
    ...
    if test_result['feasible'] and hist_state is not None and pa is not None:
        plot_comprehensive_diagnostics(hist_state, time_steps, v_x_ref, v_z_ref, pa)
