...
harmonics = BPF * np.array([1, 2, 3, 4, 5, 6])
...
freq_grid = np.linspace(min(harmonics), max(harmonics), 200)

Theta, Freq = np.meshgrid(theta_grid, freq_grid)
SPL_fit_grid = harmonic_directivity((Theta.ravel(), Freq.ravel()), *popt).reshape(Theta.shape)
...
unique_thetas = np.unique(theta_rad)  # one value per microphone
...
for i, th in enumerate(unique_thetas):
    ...
    freq_plot = np.linspace(min(harmonics), max(harmonics), 200)
    theta_plot_fixed = np.full_like(freq_plot, th)
    spl_plot = harmonic_directivity((theta_plot_fixed, freq_plot), *popt)
    plt.plot(freq_plot, spl_plot, '--', color=f"C{i}")
