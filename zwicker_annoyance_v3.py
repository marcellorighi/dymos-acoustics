...
def compute_zwicker_indicators(signal: np.ndarray, fs: int, stationary: bool = True,
                                use_fs_approximation: bool = False,
                                loudness_floor_sone: float = 0.01) -> dict:
    """
    ...
    loudness_floor_sone : float, default 0.01
        If the computed loudness falls at or below this floor (i.e. the
        signal/segment is effectively silent), sharpness is mathematically
        ill-defined (it divides by total loudness N) and MOSQITO returns
        NaN with a "invalid value encountered in divide" RuntimeWarning.
        Below this floor, sharpness/roughness/fluctuation/PA are instead
        explicitly set to NaN (and the underlying MOSQITO call for
        sharpness is skipped), turning a noisy internal NaN into an
        intentional, documented one. Set to 0 to disable this guard
        entirely (not recommended -- you'll just get the same NaNs back
        from MOSQITO, with no warning logic decision being made on purpose).
    ...
    if stationary:
        ...
        if N_val <= loudness_floor_sone:
            # Effectively silent: sharpness (which divides by N) is
            # ill-defined here, and roughness/fluctuation/PA are not
            # meaningful either. Return explicit NaNs instead of letting
            # MOSQITO's internal 0/0 NaN (with its RuntimeWarning) leak
            # through silently. See loudness_floor_sone docstring above.
            return {
                "loudness_sone": N_val,
                "sharpness_acum": float("nan"),
                "roughness_asper": float("nan"),
                "fluctuation_vacil": float("nan"),
                "annoyance_PA": float("nan"),
            }
...
def compute_zwicker_indicators_windowed(signal: np.ndarray, fs: int,
                                         ...
                                         use_fs_approximation: bool = False,
                                         loudness_floor_sone: float = 0.01,
                                         r: np.ndarray = None,
                                         min_distance_m: float = None,
                                         max_distance_m: float = None) -> dict:
    """
    ...
    r : 1D array-like, optional
        Drone-to-observer distance [m], SAME LENGTH AND TIME GRID as
        `signal` (e.g. spl_fine['r']). Required if min_distance_m or
        max_distance_m is given.
    min_distance_m, max_distance_m : float, optional
        If given (together with `r`), restrict the analysis to windows
        whose mean distance falls within [min_distance_m, max_distance_m].
        Windows outside this range are SKIPPED ENTIRELY (no MOSQITO call
        made -- cheaper than computing-then-discarding) and all their
        indicator values are left as NaN. Useful for focusing the analysis
        on the geometrically relevant part of a flyover (e.g. the
        closest-approach passage) instead of the whole, mostly-irrelevant
        trajectory. Either bound can be omitted/left None to leave that
        side unrestricted.
    ...
    dict with keys:
        ...
        'r_center'           : mean distance per window [m] (NaN if r not given)
    ...
    if (min_distance_m is not None or max_distance_m is not None) and r is None:
        raise ValueError("r must be provided to use min_distance_m/max_distance_m filtering.")
    if r is not None:
        r = np.asarray(r, dtype=float).ravel()
        if r.size != n:
            raise ValueError(
                f"r has {r.size} samples but signal has {n}; they must share the same time grid."
            )

    starts = np.arange(0, n - win_n + 1, hop_n)
    t_center = (starts + win_n / 2.0) / fs
    r_center = np.full(len(starts), np.nan)
    ...
    for i, s0 in enumerate(starts):
        if r is not None:
            r_win = r[s0:s0 + win_n]
            r_center[i] = float(np.mean(r_win))
            if min_distance_m is not None and r_center[i] < min_distance_m:
                n_skipped_distance += 1
                continue
            if max_distance_m is not None and r_center[i] > max_distance_m:
                n_skipped_distance += 1
                continue
        ...
        ind = compute_zwicker_indicators(
            ...
            loudness_floor_sone=loudness_floor_sone,
        )
        for k in keys:
            v = ind.get(k, None)
            if v is not None:
    ...
    out["r_center"] = r_center
    if n_skipped_distance > 0:
        import warnings
        warnings.warn(
            f"{n_skipped_distance}/{len(starts)} windows fell outside "
            f"[{min_distance_m}, {max_distance_m}] m and were skipped "
            "(left as NaN)."
        )

    out["t_center"] = t_center
    n_nan_pa = int(np.isnan(out["annoyance_PA"]).sum())
    n_nan_from_loudness = n_nan_pa - n_skipped_distance
    if n_nan_from_loudness > 0:
        import warnings
        warnings.warn(
            f"{n_nan_from_loudness}/{len(starts)} windows (within the "
            "distance range analyzed) produced NaN annoyance_PA, likely "
            f"near-silent windows below loudness_floor_sone="
            f"{loudness_floor_sone}. Use np.nanmean/np.nanmax or mask "
            "these out, or call fill_nan_gaps(out) to smooth over them "
            "for plotting."
        )
    return out


def fill_nan_gaps(windowed_result: dict, keys=None) -> dict:
    """
    Linearly interpolate over NaN gaps in a compute_zwicker_indicators_windowed()
    result, for cleaner plotting (e.g. of near-silent windows at the start/
    end of a flyover). Returns a new dict; does not modify the input.

    NOTE: this is purely cosmetic for visualization -- it does not recover
    any real information about what the indicator value "would have been"
    in a silent window (there usually isn't a meaningful value there).
    Avoid using interpolated values in further quantitative analysis
    (e.g. computing a mean annoyance over the whole flight) without
    considering whether silent stretches should just be excluded instead.
    """
    if keys is None:
        keys = ["loudness_sone", "sharpness_acum", "roughness_asper",
                "fluctuation_vacil", "annoyance_PA"]
    t = windowed_result["t_center"]
    out = dict(windowed_result)
    for k in keys:
        y = np.asarray(windowed_result[k], dtype=float).copy()
        nan_mask = np.isnan(y)
        if nan_mask.any() and not nan_mask.all():
            y[nan_mask] = np.interp(t[nan_mask], t[~nan_mask], y[~nan_mask])
        out[k] = y
    return out


def prepare_surrogate_targets(windowed_result: dict, keys=None) -> dict:
    """
    Convert a compute_zwicker_indicators_windowed() result into clean
    targets suitable for surrogate-model training (e.g. mapping drone
    state/acceleration -> PA for trajectory/control optimization).

    NaN windows (inaudible: loudness below the floor, or below
    min_distance_m/skipped) are replaced with 0 -- not as a fictitious
    filler value, but because the physically correct annoyance for an
    inaudible sound genuinely IS zero. Discarding these points instead
    would bias the training set away from the quiet states an optimizer
    might otherwise explore freely, and would leave the surrogate
    undefined exactly where a gradient-based trajectory/control optimizer
    could exploit that blind spot.

    The original NaN locations are preserved in 'was_nan' for diagnostics
    (e.g. to check what fraction of your dataset was inaudible, or to
    apply a different/smoothed treatment later if a hard 0 turns out to
    cause optimization issues).

    Parameters
    ----------
    windowed_result : dict
        Output of compute_zwicker_indicators_windowed().
    keys : list of str, optional
        Which indicator keys to clean. Defaults to all five Zwicker
        indicators (loudness, sharpness, roughness, fluctuation, PA).

    Returns
    -------
    dict
        Same structure as the input, with NaNs in the selected keys
        replaced by 0.0, plus a 'was_nan' sub-dict (one boolean array per
        cleaned key) recording which entries were originally NaN.
    """
    if keys is None:
        keys = ["loudness_sone", "sharpness_acum", "roughness_asper",
                "fluctuation_vacil", "annoyance_PA"]
