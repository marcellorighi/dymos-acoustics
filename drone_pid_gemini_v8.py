...
if __name__ == "__main__":
    ...
    problem = {
        ...
        'bounds': [
            [0.05, 0.25],   # Kp_Velocity (Narrowed)
            [0.01, 0.08],   # Ki_Velocity (Narrowed)
            [2.0,  4.0],    # Kp_Attitude (Kept away from extreme instability)
            [0.3,  0.8],    # Kd_Attitude (Ensured decent damping)
            [10.0, 20.0],   # Kp_Altitude
            [4.0,  10.0]    # Kd_Altitude
        ]
    }

    # 2. Generate the strict Saltelli sample matrix
    N = 2 #256  # Base sample size (ideally a power of 2)
    # Generates the required cross-sampled configurations
    param_values = sobol_sampler.sample(problem, N, calc_second_order=True)

    print(f"Generated {param_values.shape[0]} unique configurations for Sobol Analysis.")

    # Create an array to hold just the scalar outputs for SALib
    Y_max_annoyance = np.zeros(param_values.shape[0])
    Y_loudness      = np.zeros(param_values.shape[0])
    Y_sharpness     = np.zeros(param_values.shape[0])
    Y_roughness     = np.zeros(param_values.shape[0])
    Y_fluctuation   = np.zeros(param_values.shape[0])

    print("Beginning Sobol Simulation Loop...")
    for i, full_gains in enumerate(tqdm(param_values, desc="Simulating Trajectories")):
        try:
            ...
            # Store the scalar metric directly into the output vector
            #Y_max_annoyance[i] = float(np.nanmax(pa["annoyance_PA"]))
                
            annoyance_time_signal = np.asarray(pa["annoyance_PA"], dtype=float)
            loudness_time_signal = np.asarray(pa["loudness_sone"], dtype=float)
            sharpness_time_signal = np.asarray(pa["sharpness_acum"], dtype=float)
            roughness_time_signal = np.asarray(pa["roughness_asper"], dtype=float)
            fluctuation_time_signal = np.asarray(pa["fluctuation_vacil"], dtype=float)
        
            if np.any(np.isnan(hist_state)) or np.any(np.isinf(hist_state)):
                raise ValueError("Diverged")
                
            # --- NEW OPTION A: Linear Time Integral (Total Dose) ---
            # Computes the true mathematical area under the PA curve over the flight duration
            # Unit: [PA * seconds]
            pa_dose = np.trapezoid(annoyance_time_signal, x=time_steps)
            loudness_dose = np.trapezoid(loudness_time_signal, x=time_steps)
            sharpness_dose = np.trapezoid(sharpness_time_signal, x=time_steps)
            roughness_dose = np.trapezoid(roughness_time_signal, x=time_steps)
            fluctuation_dose = np.trapezoid(fluctuation_time_signal, x=time_steps)
            
            # --- NEW OPTION B: Time-Weighted Perceptual Dose (Energy Equivalent) ---
            # Humans experience loudness logarithmically. Sometimes it is preferred to square 
            # the metric before integrating to heavily penalize loud durations.
            # pa_dose = np.trapezoid(annoyance_time_signal**2, x=time_steps)

            # Feed the Dose array into your Sobol sensitivity tracker
            Y_max_annoyance[i] = float(pa_dose)
            Y_loudness[i] = float(loudness_dose)
                            
        except Exception:
            # Use a high penalty value for unstable runs
            Y_max_annoyance[i] = 99.0 
            Y_loudness[i]= 99.0 

    print("Simulations complete. Evaluating variance decompositions...")

    # Compute first-order (S1), second-order (S2), and total-order (ST) indices
    Si = sobol_analyzer.analyze(problem, Y_max_annoyance, calc_second_order=True)
    Si_L = sobol_analyzer.analyze(problem, Y_loudness, calc_second_order=True)

    # Cleanly display First-Order and Total-Order sensitivity
    for i, name in enumerate(problem['names']):
        print(f"{name:15s} | First-Order (S1): {Si['S1'][i]:.3f} | Total-Order (ST): {Si['ST'][i]:.3f}")
    for i, name in enumerate(problem['names']):
        print(f"{name:15s} | First-Order (S1): {Si_L['S1'][i]:.3f} | Total-Order (ST): {Si_L['ST'][i]:.3f}")

    num_penalties = np.sum(Y_max_annoyance == 99.0)
    print(f"--- DATA DIAGNOSTICS ---")
    print(f"Total runs evaluated: {len(Y_max_annoyance)}")
    print(f"Number of failed/unstable runs (penalized with 99.0): {num_penalties}")
    print(f"Unique values in Y: {np.unique(Y_max_annoyance)}")
    print(f"Standard Deviation of Y: {np.std(Y_max_annoyance)}")
    print(f"------------------------")
