...
class AcousticParams:
    ...
    c_sound: float = 343.0      # [m/s]  speed of sound, used for power -> pressure conversion
    rho: float = 1.225          # [kg/m^3] air density, used for power -> pressure conversion
...
def power_to_pressure_amplitude(p_acoustic: np.ndarray, r: np.ndarray, theta_rad: np.ndarray,
                                 params: AcousticParams) -> np.ndarray:
    """
    Convert radiated acoustic power [W] from a single rotor into a peak
    pressure amplitude [Pa] at distance r [m] and angle theta [rad] from the
    rotor axis, via free-field intensity:
        I(r, theta) = P * D(theta) / (4*pi*r^2)        [W/m^2]
        p_rms       = sqrt(I * rho * c)                [Pa]
        p_peak      = sqrt(2) * p_rms                  [Pa]  (for a sinusoidal carrier)
    This is the physically consistent way to scale a unit-amplitude sin()
    carrier into an actual pressure signal, instead of multiplying by power
    directly (which would carry units of Watts, not Pascals, and would
    ignore that pressure also depends on distance and directivity).
    """
...
class FineGridParams:
    ...
    disturbance_amplitude_rad: float = 0.05
    ...
    disturbance_bandwidth_hz: float = 20.0
    ...
    random_seed: int = None       # set for reproducible disturbance realizations
...
def generate_rotor_azimuth(rpm_fine: np.ndarray, t_fine: np.ndarray,
    ...
    """
    Generate one rotor's synthetic acoustic pressure signal on the fine time
    grid: a sinusoidal carrier at the rotor's (instantaneous) rotation
    frequency, phase-jittered by a small disturbance, and scaled to a
    physically-consistent pressure amplitude [Pa] using the rotor's
    radiated acoustic power, the observer distance r, and the directivity
    angle theta.

        p_rotor(t) = p_peak(t) * sin(phase(t) + disturbance(t))

    where p_peak(t) comes from power_to_pressure_amplitude() (see that
    function's docstring for the power -> pressure derivation). Note this
    is still a single-tone (shaft-rotation-frequency) approximation, not a
    full blade-passage-frequency synthesis -- see phase's docstring note
    below for how to extend it.

    Parameters
    ----------
    rpm_fine : array-like
        This rotor's RPM time series, already interpolated onto t_fine.
    t_fine : array-like
        Fine time grid [s].
    r : array-like
        Distance from rotor to observer at each time [m] (same length as
        t_fine). From observer_geometry().
    theta_rad : array-like
        Angle from the rotor axis to the observer at each time [rad] (same
        length as t_fine). From observer_geometry().
    acoustic_params : AcousticParams
        RPM-to-power, directivity, and propagation assumptions.
    fine_params : FineGridParams, optional
        Phase-integration method and disturbance assumptions.
    rng : numpy Generator, optional

    Returns
    -------
    p_rotor : ndarray
        Synthetic pressure signal from this rotor at the observer [Pa].
    phase : ndarray
        The underlying (un-disturbed) phase [rad], returned for diagnostics
        / use in synthesizing blade-passage-frequency tones later
        (e.g. sin(n_blades * phase) for the n-th blade harmonic, which
        would also need its own directivity/power treatment per harmonic --
        not implemented here, this function gives only the fundamental).
    disturbance : ndarray
    ...
    p_peak = power_to_pressure_amplitude(p_acoustic, r, theta_rad, acoustic_params)
...
input_path = DEFAULT_INPUT_DIR / "directivity_model_pchip.npz"

def load_directivity_model(path=input_path):
    data = np.load(path)
    harmonic_numbers = data["harmonic_numbers"]
    # freqs = data["freqs"]
    coeffs = data["coeffs"]
    coeff_names = list(data["coeff_names"])

    smooth_coeffs = {
    name: PchipInterpolator(harmonic_numbers, coeffs[:, k])
    for k, name in enumerate(coeff_names)
    }

    # smooth_coeffs = {
    #     name: PchipInterpolator(freqs, coeffs[:, k])
    #     for k, name in enumerate(coeff_names)
    # }

    def harmonic_directivity_smooth(theta, m):
        A  = smooth_coeffs["A"](m)
        a1 = smooth_coeffs["a1"](m); b1 = smooth_coeffs["b1"](m)
        a2 = smooth_coeffs["a2"](m); b2 = smooth_coeffs["b2"](m)
...
def generate_rotor_azimuth_with_harmonics(rpm_fine, t_fine, r, theta_rad,
                                           ...
                                           directivity_model=None,   # harmonic_directivity_smooth
                                           theta_ref: float = 0.0):
    if fine_params is None:
        fine_params = FineGridParams()
    if rng is None:
    ...
    # reference: fundamental harmonic, reference angle -> anchor for relative scaling
    ref_level_db = harmonic_directivity_smooth(theta_ref, 1.0)

    p_rotor = np.zeros_like(t_fine, dtype=float)

    for n in range(1, max_harmonics + 1):
        level_db = harmonic_directivity_smooth(theta_rad, float(n))          # empirical shape: angle + harmonic
        amp_ratio = 10.0 ** ((level_db - ref_level_db) / 20.0)      # replaces harmonic_scale entirely
...
def generate_shaped_broadband_noise(p_broadband, r, t_fine, fs, f_peak=1500.0, bandwidth_oct=1.5, rng=None):
    if rng is None:
        rng = np.random.default_rng()

    # 1. Target RMS pressure at distance r [Pa]
    # (1.21 kg/m^3 = air density, 343 m/s = speed of sound)
    p_bb_rms_target = np.sqrt((1.21 * 343.0 * p_broadband) / (4.0 * np.pi * (r**2)))
    
    # 2. Generate raw white noise
    raw_noise = rng.normal(0.0, 1.0, size=len(t_fine))
    
    # 3. Calculate lower and upper cutoff frequencies around f_peak
    f_low = max(100.0, f_peak / (2**(bandwidth_oct / 2)))
    f_high = min(fs / 2.1, f_peak * (2**(bandwidth_oct / 2)))
    
    # 4. Filter the noise to form the characteristic broadband hump
    sos = butter(N=2, Wn=[f_low, f_high], btype='bandpass', fs=fs, output='sos')
    shaped_noise = sosfilt(sos, raw_noise)
    
    # 5. Rescale normalized filtered noise to match exact target RMS power
    current_rms = np.std(shaped_noise)
    if current_rms > 0:
        shaped_noise = shaped_noise * (p_bb_rms_target / current_rms)
        
    return shaped_noise

def dummy_broadband_acoustic_power(
    rpm_dict: dict, 
    f_peak: float = 1500.0, 
    amp_factor: float = 1.0,
    width_octaves: float = 1.5
) -> float:
    """
    Generates a dummy broadband acoustic power [Watts] based on average rotor RPM.
    
    Parameters
    ----------
    rpm_dict : dict
        Dictionary of rotor RPMs (e.g., {'front': ..., 'rear': ...}).
    f_peak : float
        Peak frequency of the broadband noise hump [Hz] (e.g. 1000 - 2000 Hz).
    amp_factor : float
        Tuning multiplier to scale the broadband noise power relative to tonal.
    width_octaves : float
        Spectral width/spread of the broadband hump.

    Returns
    -------
    p_broadband : array-like or float
        Broadband acoustic power [W] integrated across the spectrum.
    """
    # 1. Compute average RPM across all rotors
    avg_rpm = np.mean([np.asarray(v) for v in rpm_dict.values()], axis=0)
    
    # 2. Scale baseline power using standard acoustic power-law (U^5 to U^6 scaling)
    # At 5000 RPM, baseline broadband power is ~1 mW per rotor (scaled by amp_factor)
    p_broadband_base = amp_factor * 1e-3 * (avg_rpm / 5000.0)**5.5
    
    # Return total power across 4 rotors [W]
    return np.clip(p_broadband_base * len(rpm_dict), 1e-12, None)
...
def estimate_received_spl_fine(t, x, y, z,
    ...
    # --- 3. observer geometry (needed before generating pressure signals,
    #         since pressure amplitude depends on distance and directivity) ---
    r, theta = observer_geometry(x_fine, y_fine, z_fine, observer_xyz, z_up=z_up)

    # --- 4. per-rotor synthetic pressure signal (with disturbance) ---
    rng = np.random.default_rng(fine_params.random_seed)
    ...
    # --- 5. sound power / SPL, same model as estimate_received_spl(), on t_fine ---
    p_total = sum(acoustic_power_per_rotor(rpm_fine[name], acoustic_params) for name in rpm_fine)
    swl_db = 10.0 * np.log10(p_total / 1e-12)

    # D = directivity_factor(theta)
    # spl_db = swl_db + 10.0 * np.log10(D) - 20.0 * np.log10(r) - 11.0
    spl_db = swl_db - 20.0 * np.log10(r) - 11.0

    # --- 6. combined synthetic pressure signal at the observer ---
    # Rotors are summed as INCOHERENT sources for the SPL/power budget above
    # (see module docstring), but here we also provide the literal sum of
    # the per-rotor pressure signals -- a simple coherent superposition --
    # since that is what you'd actually feed to a time-domain psychoacoustic
    # analysis (e.g. compute_zwicker_indicators()). Note this pressure-level
    # sum is an approximation: it doesn't enforce the same incoherent power
    # budget as p_total/spl_db above, since the 4 rotors' phases are
    # generally not synchronized. For 4 similar rotors this is usually a
    # reasonable engineering approximation, but the two should be expected
    # to differ somewhat in their implied RMS level.
    p_signal = sum(p_rotor[name] for name in p_rotor)

    # 🟢 ADD BROADBAND POWER NOISE HERE
    if include_broadband:
        p_broadband = dummy_broadband_acoustic_power(
            rpm_fine, 
            f_peak=bb_f_peak, 
            amp_factor=bb_amp_factor
        )
        p_total += p_broadband
    else:
        p_broadband = 0.0
    ...
    swl_db = 10.0 * np.log10(np.maximum(p_total, 1e-12) / 1e-12)

    # Sound Pressure Level [dB] at observer distance r
    spl_db = swl_db - 20.0 * np.log10(np.maximum(r, 0.1)) - 11.0

    # --- 6. Combined synthetic pressure signal at observer ---
    # Sum of discrete blade tones
    p_tonal_signal = sum(p_rotor[name] for name in p_rotor)
    
    # Optional: Add time-domain random Gaussian white noise scaled to match broadband power
    if include_broadband:
        # Convert broadband power to equivalent RMS pressure at distance r
        # p_bb_rms = np.sqrt((1.21 * 343.0 * p_broadband) / (4.0 * np.pi * (r**2)))
        # p_bb_signal = rng.normal(0.0, p_bb_rms, size=len(t_fine))
        p_bb_signal = generate_shaped_broadband_noise(
            p_broadband=p_broadband,
            r=r,
            t_fine=t_fine,
            fs=fine_params.fs,
            f_peak=bb_f_peak,
        ...
        p_signal = p_tonal_signal + p_bb_signal
    else:
        p_signal = p_tonal_signal
    ...
    for name in rpm_fine:
        ...
        result[f"p_rotor_{name}"] = p_rotor[name]
...
if __name__ == "__main__":
    ...
    print(f"  p_rotor_front  : {spl_fine['p_rotor_front'].min():.4f} - {spl_fine['p_rotor_front'].max():.4f} Pa")
    print(f"  p_signal (sum) : {spl_fine['p_signal'].min():.4f} - {spl_fine['p_signal'].max():.4f} Pa")