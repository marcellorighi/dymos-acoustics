...
if __name__ == "__main__":
    N = 1  # Set low for rapid pipeline validation, scale up to 128+ later
    ...
    # 🟢 RUN THIS ON THE CLUSTER TO WRITE THE JSON FILE
    import json
    ...
    for metric, display_name in metrics_to_analyze.items():
        # Run the SALib analyzer on your cluster simulation outputs
        Si = sobol_analyzer.analyze(problem, Y_metrics[metric])
        
        # Package arrays into lists so JSON can serialize them cleanly
        sobol_export_data[metric] = {
            "names": list(problem['names']),
            "S1": list(Si['S1']),
            "ST": list(Si['ST']),
            # Convert S2 matrix to a list of lists if it exists, otherwise keep None
            "S2": Si['S2'].tolist() if ('S2' in Si and Si['S2'] is not None) else None
        }

    # Save the structured dictionary down to disk
    json_filename = "sobol_raw_data.json"
    with open(json_filename, 'w') as json_file:
        json.dump(sobol_export_data, json_file, indent=4)

    print(f"✅ Raw Sobol structural data successfully saved to: {json_filename}")
