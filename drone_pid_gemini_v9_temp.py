...
if __name__ == "__main__":
    ...
    # 1. Define the indicators we want to track
    # Mapping the dict key in 'pa' to a clean display name
    metrics_to_analyze = {
    ...
    # 2. Pre-allocate an output dictionary of arrays to store the results
    # Each metric gets an array of size equal to the number of simulations
    num_simulations = param_values.shape[0]
    Y_metrics = {metric: np.zeros(num_simulations) for metric in metrics_to_analyze.keys()}

    # Define a penalty value for unstable runs 
    # (Set this reasonably close to or slightly above your expected maximum doses)
    PENALTY_VALUE = 99.0

    print("Beginning Multi-Indicator Sobol Simulation Loop...")
    ...
    for i, full_gains in enumerate(tqdm(param_values, desc="Simulating Trajectories")):
        try:
            ...
            # Immediate divergence guard
            if np.any(np.isnan(hist_state)) or np.any(np.isinf(hist_state)):
                raise ValueError("Diverged")
                
            # Loop through each psychoacoustic indicator automatically
            for metric in metrics_to_analyze.keys():
                # Extract time-series signal safely
                time_signal = np.asarray(pa[metric], dtype=float)
                
                # Compute integrated dose over time
                dose_value = np.trapezoid(time_signal, x=time_steps)
                
                # Guard against any edge-case NaNs in the acoustic output itself
                if np.isnan(dose_value) or np.isnan(time_signal).any():
                    raise ValueError("Acoustic output contained NaN values")
                    
                Y_metrics[metric][i] = float(dose_value)
                
        except Exception as e:
            # --- ADD THIS DETAILED ERROR LOGGER ---
            import traceback
            print(f"\n🛑 RUN {i} FAILED AT CROSSING 3 SECONDS!")
            ...
            # Keep the penalty assignment so the script doesn't completely stop yet
            for metric in metrics_to_analyze.keys():
                Y_metrics[metric][i] = PENALTY_VALUE
                
            # Stop the loop after printing the first failure so your screen isn't flooded
            print("Stopping execution loop early to let you review the error above.")
            break

    print("\nSimulations complete. Running Sobol sensitivity decomposition...")
    ...
    #     # Run the SALib analyzer on the specific indicator vector
    #     Si = sobol_analyzer.analyze(problem, Y_metrics[metric], calc_second_order=True)
    #     sobol_results[metric] = Si  # Store it for potential plotting later
        
    #     # Print the First-Order and Total-Order tables neatly
    #     print(f"{'Gain Parameter':20s} | {'First-Order (S1)':<16} | {'Total-Order (ST)':<16}")
    #     print("-" * 60)
    #     for idx, name in enumerate(problem['names']):
    #         print(f"{name:20s} | {Si['S1'][idx]:16.3f} | {Si['ST'][idx]:16.3f}")

    # num_penalties = np.sum(Y_max_annoyance == PENALTY_VALUE)
    # print(f"--- DATA DIAGNOSTICS ---")
    # print(f"Total runs evaluated: {len(Y_max_annoyance)}")
    # print(f"Number of failed/unstable runs (penalized with PENALTY_VALUE): {num_penalties}")
    # print(f"Unique values in Y: {np.unique(Y_max_annoyance)}")
    # print(f"Standard Deviation of Y: {np.std(Y_max_annoyance)}")
    # print(f"------------------------")

    # 2. Export everything safely to a CSV file
    # df_results = pd.DataFrame(compiled_rows)
    # df_results.to_csv("sobol_simulation_results.csv", index=False)
    # print("All simulation data successfully saved to 'sobol_simulation_results.csv'!")

    # 3. Automatically analyze and print results for each indicator
    sobol_results = {}
