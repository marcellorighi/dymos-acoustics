    ...
    for i, full_gains in enumerate(tqdm(param_values, desc="Simulating Trajectories")):
        try:
            ...
            for metric in metrics_to_analyze.keys():
                # Extract time-series signal safely (this has length e.g. 2)
                time_signal = np.asarray(pa[metric], dtype=float)
                
                # --- FIX: Create a time grid that matches the length of the windows ---
                # This spaces out the integration points evenly across your total flight duration
                acoustic_time_grid = np.linspace(0, time_steps[-1], len(time_signal))
                
                # Compute integrated dose using the matching grid
                dose_value = np.trapezoid(time_signal, x=acoustic_time_grid)
            ...
            # If the simulation or calculation crashes, penalize ALL tracked metrics for this run
            for metric in metrics_to_analyze.keys():
