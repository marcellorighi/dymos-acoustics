...
def compute_zwicker_indicators_windowed(signal: np.ndarray, fs: int,
    ...
    if n_nan_pa > 0:
        import warnings
        warnings.warn(
            f"{n_nan_pa}/{len(starts)} windows produced NaN annoyance_PA "
            f"(likely near-silent windows below loudness_floor_sone="
            f"{loudness_floor_sone}). Use np.nanmean/np.nanmax or mask "
            "these out, or pass interpolate_nans=True via "
            "fill_nan_gaps(out) to smooth over them for plotting."
        )
    return out
