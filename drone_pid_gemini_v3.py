...
class DronePIDController:
    ...
    def compute_controls_velocity_tracking(self, state, target_velocity, target_yaw):
        """
        Docstring for compute_controls_velocity_tracking
        
        :param self: Description
        :param state: Description
        :param target_velocity: Description
        :param target_yaw: Description
        """
        kp_vel_z = 12.0 
        ...
        # 1. FLIP THIS SIGN: Change -1.0 to +1.0 to match your rotation matrix convention
        # Since negative theta is driving the drone backward, we need a POSITIVE theta to go forward.
        kp_vel_tuning = 0.1  # 2. LOWERED GAIN to stop immediate hard-clipping
        theta_cmd = +1.0 * (kp_vel_tuning * error_vx) 
        
        # Mirror the change for roll (flip sign if Y behaves backward too)
        phi_cmd   = +1.0 * (kp_vel_tuning * error_vy) 
        
        # Keep the safety clamp active
        theta_cmd = np.clip(theta_cmd, -0.5, 0.5)
        phi_cmd   = np.clip(phi_cmd, -0.5, 0.5)
...
if __name__ == "__main__":
    ...
    print("Running uniform velocity simulation and capturing flight telemetry...")
    for idx, t in enumerate(time_steps):
        
        # CHANGE HERE: Call the velocity tracking controller instead of the position controller
        current_controls, theta_cmd, phi_cmd = controller.compute_controls_velocity_tracking(state, target_velocity, target_yaw)
        
        # The RK4 integration step remains exactly the same
